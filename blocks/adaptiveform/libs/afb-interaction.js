...
export const subscribe = (model, element, additionalHandlers) => {
  ...
  model?.subscribe((/** @type {{ target: { getState: () => any; }; payload: { changes: any; }; }} */ action) => {
    const state = action.target.getState();
    const { changes } = action.payload;
    changes.forEach((/** @type {{ propertyName: string; prevValue: any; currentValue: any; }} */ change) => {
...
export const updateErrorMessage = (/** @type {{ querySelector: (arg0: string) => any; }} */ element, /** @type {any} */ errorMessage, /** @type {{ errorMessage: any; }} */ state) => {
  if (element) {
    const errorDiv = getErrorWidget(element);
